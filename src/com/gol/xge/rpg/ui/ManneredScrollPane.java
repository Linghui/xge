...
public class ManneredScrollPane extends ScrollPane implements
		PageOperateListener {
	...
	private boolean horizontal = false;// by default, add items to bottom, if
										// set to true then add item to the
										// right
	...
	public void act(float delta) {
		super.act(delta);
		if (!Gdx.input.isTouched() && touched && !this.isFlinging()) {
			touched = false;
	...
	public void addActor(Actor actor) {
		...
		if (horizontal) {
			...
			if (lineActors.getChildren().size < this.pageSize) {
				...
				// Gdx.app.log(TAG, "dis " + dis + " cellSizeHeight " +
				// cellSizeHeight);
	 ...
	 * 这里使用方法特殊，当滚动设置为上下滚动，并且元素数量少于每页显示数量的时候， 调整完页面内元素个数后，一定要显示给定一个》0的y值
