...
//    private float secondsDown = 0f;
//    private float timeOfScale = 0.2f;
...
//    private float preY = 0f;
    ...
    public CoreImage (Texture texture) {
        this(new TextureRegion(texture));
    }

    /** Creates an image aligned center. */
    public CoreImage (Texture texture, Scaling scaling) {
        this(new TextureRegion(texture), scaling);
    }

    public CoreImage (Texture texture, Scaling scaling, int align) {
        this(new TextureRegion(texture), scaling, align);
    }

    public CoreImage (Texture texture, Scaling scaling, int align, String name) {
        this(new TextureRegion(texture), scaling, align, name);
    }

    /** Creates an image stretched, and aligned center.
     * @param region May be null. */
    public CoreImage (TextureRegion region) {
        this(region, Scaling.stretch, Align.CENTER, null);
    }

    /** Creates an image aligned center.
     * @param region May be null. */
    public CoreImage (TextureRegion region, Scaling scaling) {
        this(region, scaling, Align.CENTER, null);
    }

    /** @param region May be null. */
    public CoreImage (TextureRegion region, Scaling scaling, int align) {
        this(region, scaling, align, null);
    }

    /** @param region May be null. */
    public CoreImage (TextureRegion region, Scaling scaling, int align, String name) {
        super(name);
        setRegion(region);
    ...
    public CoreImage (NinePatch patch) {
        this(patch, Scaling.stretch, Align.CENTER, null);
    }

    /** Creates an image aligned center.
     * @param patch May be null. */
    public CoreImage (NinePatch patch, Scaling scaling) {
        this(patch, scaling, Align.CENTER, null);
    }

    /** @param patch May be null. */
    public CoreImage (NinePatch patch, Scaling scaling, int align) {
        this(patch, scaling, align, null);
    }

    /** @param patch May be null. */
    public CoreImage (NinePatch patch, Scaling scaling, int align, String name) {
        super(name);
        setPatch(patch);
        this.scaling = scaling;
        this.align = align;
    ...
    public void draw (SpriteBatch batch, float parentAlpha) {
        ...
        if (patch != null)
            ...
            if (scaleX == 1 && scaleY == 1 && rotation == 0)
                batch.draw(region, x + imageX, y + imageY, Math.abs(imageWidth), Math.abs(imageHeight));
            else
...
//        super.act(delta);
//        if(draggable && isDown && actionSet == false){
//            secondsDown += Gdx.graphics.getDeltaTime();
//            if(secondsDown > timeOfScale){
//                this.action(ScaleTo.$(scaleXY, scaleXY, scaleDuration));
//                actionSet = true;
////                List<Actor> actors = parent.parent.getActors();
////                boolean temp = this.parent.parent.swapActor(this.parent, actors.get(actors.size() - 1));
////                Gdx.app.log(TAG, "swap + " + temp);
//            }
//        }
//    }
//    
...
//        isDown = true;
////        boolean temp = hit(x,y) == null;
////        Gdx.app.log(TAG, "hit ? " + temp);
//        if(draggable){
//            preX = this.x;
//            preY = this.y;
//        }
