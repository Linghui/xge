...
public class XGECommon {
	...
	public static AnimationGroup readAnimationGroup(JsonValue json,
			TextureAtlas atlas) {
		AnimationResource ar = new AnimationResource(json);
		return readAnimationGroup(ar, atlas, 0.1f);
	}

	public static AnimationGroup readAnimationGroup(AnimationResource ar,
			TextureAtlas atlas, float speed) {

//		HashMap<String, Animation> animationHash = AnimationDataCenter
//				.getInstance().get(atlas);
		HashMap<String, Animation> animationHash = null;

		if (animationHash != null) {
			return new AnimationGroup(animationHash, ar.getOffsetX(),
					ar.getOffsetY());
		} else {
			animationHash = new HashMap<String, Animation>();
		}
		...
		for (JsonValue action = actionsObj.child(); action != null; action = action
				.next()) {
			String actionName = action.name();

			if (actionName.equals("offsetX") || actionName.equals("offsetY")) {
				continue;
			}

			List<TextureRegion> actionAnimation = new ArrayList<TextureRegion>();

			for (JsonValue texIds = action.child(); texIds != null; texIds = texIds
					.next()) {
		...
		AnimationDataCenter.getInstance().save(atlas, animationHash);

		AnimationGroup group = new AnimationGroup(animationHash,
				ar.getOffsetX(), ar.getOffsetY());
		return group;
	}
	...
	public static void timeStamp(String tag) {
		...
		System.out.println("time stamp - " + calendar.get(Calendar.MINUTE)
